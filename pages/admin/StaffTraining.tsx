...
export const StaffTraining: React.FC = () => {
  ...
  const fetchStaffVideos = useCallback(async () => {
    setIsLoading(true);
    try {
      const staffId = currentUser?.staffId || currentUser?.id || 'staff_user';
      const res = await fetch(`/api/training-videos?role=staff&user_id=${staffId}&user_type=staff`);
      if (!res.ok) throw new Error('Failed to load training videos');
  ...
  const handleMarkCompleted = async (videoId: number) => {
    ...
    try {
      const res = await fetch(`/api/training-videos/${videoId}/complete`, {
        ...
        body: JSON.stringify({ user_id: staffId, user_type: 'staff' }),
      });
      if (!res.ok) throw new Error('Failed to record status');
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      {/* Hero Header */}
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-6 sm:p-8 rounded-3xl border border-slate-800 shadow-2xl">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-xl">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
              Shrawello Staff Knowledge & SOP Hub
            </h1>
            <p className="text-slate-300 text-xs sm:text-sm leading-relaxed">
              Explore step-by-step video tutorials on lead management, itinerary creation, booking workflows, and internal SOPs.
            </p>
          </div>

          {/* Progress Card */}
          <div className="bg-slate-900/90 border border-slate-800 p-4 sm:p-5 rounded-2xl min-w-[240px] space-y-3 shadow-xl">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-300">
              <span>Your Training Progress</span>
              <span className="text-blue-400 font-bold text-sm">{progressPercentage}%</span>
            </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72 shrink-0">
          ...
          <input
            type="text"
            placeholder="Search staff tutorials..."
            value={searchQuery}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-slate-900/40 border border-slate-800 rounded-2xl space-y-3">
          <span className="material-symbols-outlined text-4xl text-slate-600">play_lesson</span>
          <p className="text-slate-400 font-medium text-sm">No staff training videos available in this category.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredVideos.map((video) => (
            <div
              ...
              className="group bg-slate-900 border border-slate-800 hover:border-blue-500/50 rounded-2xl overflow-hidden shadow-lg hover:shadow-blue-500/10 cursor-pointer active:scale-[0.99] transition-all duration-300 flex flex-col justify-between"
            >
              {/* Thumbnail */}
              <div className="relative aspect-video bg-slate-950 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-slate-950/40 group-hover:bg-slate-950/20 transition-colors flex items-center justify-center">
                  <div className="size-12 rounded-full bg-blue-600/90 text-white flex items-center justify-center shadow-xl group-hover:scale-110 transition-transform">
              ...
              <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-slate-800 text-blue-400 border border-slate-700">
                    {video.category}
                  </span>
                  <h3 className="font-bold text-white text-sm sm:text-base leading-snug mt-2 line-clamp-2 group-hover:text-blue-400 transition-colors">
                    {video.title}
                  </h3>
                  {video.description && (
                ...
                <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-blue-400 font-semibold">
                  <span className="flex items-center gap-1 group-hover:underline">
                    Watch Tutorial <span className="material-symbols-outlined text-sm">arrow_forward</span>
                  </span>

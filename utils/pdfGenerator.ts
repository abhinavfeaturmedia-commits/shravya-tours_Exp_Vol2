...
export const generateProformaInvoice = async (proposal: Proposal, optionId: string, lead: Lead) => {
    ...
    // --- Company Data ---
    const company = {
        name: "Shrawello Travel Hub and Events LLP",
    ...
    const logoImg = await loadLogo();
    if (logoImg) {
        doc.addImage(logoImg, "PNG", 15, 12, 30, 28.5);
    } else {
        doc.setFontSize(18);
        doc.setFont("helvetica", "bold");
    ...
    doc.setFontSize(8);
    doc.setTextColor(150, 150, 150);
    doc.text("Thank you for choosing Shrawello Travel Hub and Events LLP!", pageWidth / 2, pageHeight - 15, { align: 'center' });

    doc.save(`Proforma_${lead.name.replace(/\s+/g, '_')}_${proposal.id}.pdf`);
};

export const generateBookingInvoice = async (booking: any, customer: any) => {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.width;
...
export const generateReceiptPDF = async (
    ...
    if (tx.status === 'Pending') {
        ...
        doc.setFontSize(6.2);
        doc.setTextColor(220, 38, 38);
        doc.text("WARNING: RECEIPT STATUS: AWAITING FINANCE VERIFICATION. THIS PAYMENT IS CURRENTLY PENDING APPROVAL.", 105, 54.8, { align: 'center' });
    }
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(100, 116, 139);
    doc.text("for choosing Shrawello Travel Hub and Events LLP!", pageWidth - 15, footerY + 9.5, { align: 'right' });
